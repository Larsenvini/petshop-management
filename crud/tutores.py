...
def criar_janela_tutores():
    ...
    ctk.CTkLabel(frame_campos, text="Telefone:").grid(row=1, column=0, padx=5, pady=5)
    entrada_telefone = ctk.CTkEntry(frame_campos)
    entrada_telefone.grid(row=1, column=1, padx=5, pady=5)

    ctk.CTkLabel(frame_campos, text="Email:").grid(row=2, column=0, padx=5, pady=5)
    entrada_email = ctk.CTkEntry(frame_campos)
    entrada_email.grid(row=2, column=1, padx=5, pady=5)

    ctk.CTkLabel(frame_campos, text="Endereço:").grid(row=3, column=0, padx=5, pady=5)
    entrada_endereco = ctk.CTkEntry(frame_campos)
    entrada_endereco.grid(row=3, column=1, padx=5, pady=5)

    ctk.CTkLabel(frame_campos, text="CPF:").grid(row=4, column=0, padx=5, pady=5)
    entrada_cpf = ctk.CTkEntry(frame_campos)
    entrada_cpf.grid(row=4, column=1, padx=5, pady=5)
    ...
    def atualizar_lista():
        ...
        for tutor in tutores:
            lista_tutores.insert("end", f"ID: {tutor[0]} - {tutor[1]} - {tutor[2]} - {tutor[5]}\n")
